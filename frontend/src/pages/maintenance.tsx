...
function isMaintenanceNear(item: {
  ...
  return millisecondsRemaining <= 3 * 86400000
}
...
export function MaintenancePage() {
  ...
  const create = useMutation({
    mutationFn: () => maintenanceService.create({ name: name.trim(), triggerType, gallonInterval: triggerType === 'GALLONS' ? Number(interval) : undefined, dayInterval: triggerType === 'DAYS' ? Number(interval) : undefined, notes: notes || undefined }),
  ...
  return <PageLayout title="Maintenance" breadcrumbItems={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Maintenance' }]}>
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <p className="text-gray-500">Total recorded gallons sold: <strong>{query.data?.totalGallonsSold ?? 0}</strong></p>
        <Button onClick={() => setOpen(true)}><FiPlus /> Add Maintenance</Button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {(query.data?.schedules ?? []).map((item) => <Card key={item.id} className={item.due ? 'border-red-500' : ''}>
          <CardContent className="p-5 space-y-3">
            <div className="flex justify-between gap-2"><div className="flex items-center gap-2"><FiTool /><h3 className="font-semibold">{item.name}</h3></div><Badge variant={item.due ? 'danger' : 'success'}>{item.due ? 'Due' : 'On schedule'}</Badge></div>
            {item.triggerType === 'GALLONS' ? <p className="text-sm">{item.gallonsSinceMaintenance} / {item.gallonInterval} gallons since maintenance<br/><span className="text-gray-500">{item.gallonsRemaining} gallons remaining</span></p> : <p className="text-sm">Every {item.dayInterval} days<br/><span className="text-gray-500">Next due: {item.nextDueAt ? new Date(item.nextDueAt).toLocaleDateString() : 'Not scheduled'}</span></p>}
      ...
      <div>
        ...
        <DataTable<MaintenanceHistoryItem>
          ...
          columns={[
            ...
            { key: 'gallonCountAtCompletion', header: 'Gallons Sold', render: item => item.gallonCountAtCompletion.toLocaleString() },
    ...
    <Modal open={open} onClose={() => setOpen(false)} title="Add Maintenance Schedule">
      <div className="space-y-4">
        ...
        <div><label className="block text-sm font-medium mb-1">Schedule based on</label><Select value={triggerType} onChange={e => { const value=e.target.value as 'GALLONS'|'DAYS'; setTriggerType(value); setInterval(value === 'GALLONS' ? '250' : '30') }} options={[{ value:'GALLONS',label:'Gallons sold'},{value:'DAYS',label:'Days / calendar'}]} /></div>
        <div><label className="block text-sm font-medium mb-1">Every how many {triggerType === 'GALLONS' ? 'gallons' : 'days'}?</label><Input type="number" min="1" value={interval} onChange={e => setInterval(e.target.value)} /></div>
        <div><label className="block text-sm font-medium mb-1">Notes</label><Input value={notes} onChange={e => setNotes(e.target.value)} /></div>
        <div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setOpen(false)}>Cancel</Button><Button loading={create.isPending} disabled={!name.trim() || Number(interval) < 1} onClick={() => create.mutate()}>Add</Button></div>
      </div>
    </Modal>
  </PageLayout>
}
