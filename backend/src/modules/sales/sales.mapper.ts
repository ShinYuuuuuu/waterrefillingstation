...
export class SaleMapper {
  ...
  static toResponse(sale: Sale, items: SaleItem[] = [], payments: SalePayment[] = []): SaleResponse {
    return {
      ...
      cashierName: (sale as Sale & { user?: { full_name: string } }).user?.full_name ?? 'Unknown cashier',
      customerName: (sale as Sale & { customer?: { full_name: string } | null }).customer?.full_name ?? 'Walk-in customer',
  ...
  static toItemResponse(item: SaleItem): SaleItemResponse {
    return {
      ...
      productName: (item as SaleItem & { product?: { name: string } }).product?.name ?? item.product_name ?? 'Product',
